...
def upgrade():
    ...
    with op.batch_alter_table('feature', schema=None) as batch_op:
        ...
        batch_op.add_column(sa.Column('geometry', Geometry(geometry_type='POINT', srid=4326, from_text='ST_GeomFromEWKT', name='geometry'), nullable=True))
        ...
        batch_op.create_index('idx_feature_geometry', ['geometry'], unique=False, postgresql_using='gist')
...
def downgrade():
    # ### commands auto generated by Alembic - please adjust! ###
    with op.batch_alter_table('feature', schema=None) as batch_op:
        batch_op.add_column(sa.Column('element_type', sa.INTEGER(), autoincrement=False, nullable=False))
    ...
    with op.batch_alter_table('challenge', schema=None) as batch_op:
        batch_op.add_column(sa.Column('feature_tags', postgresql.ARRAY(sa.VARCHAR()), autoincrement=False, nullable=False))
