...
def upgrade():
    ...
    with op.batch_alter_table('feature', schema=None) as batch_op:
        ...
        batch_op.create_index(batch_op.f('ix_feature_localized_by'), ['localized_by'], unique=False)
        batch_op.create_index(batch_op.f('ix_feature_locked_by'), ['locked_by'], unique=False)
        batch_op.create_index(batch_op.f('ix_feature_validated_by'), ['validated_by'], unique=False)
        batch_op.create_foreign_key('fk_users_localizer', 'users', ['localized_by'], ['id'])
        batch_op.create_foreign_key('fk_users_validator', 'users', ['validated_by'], ['id'])
...
def downgrade():
    ...
    with op.batch_alter_table('feature', schema=None) as batch_op:
        ...
        batch_op.alter_column('localized_by',
               existing_type=sa.BigInteger(),
               type_=sa.VARCHAR(),
               existing_nullable=True)
